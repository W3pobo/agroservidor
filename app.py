...
def login_google():
    ...
    # Agrega prompt=select_account para forzar selección de cuenta
...
def productos():
    ...
    # Obtener el tipo de usuario de la sesión y normalizar a mayúscula inicial
    ...
    return render_template('productos.html', 
                           ...
                           tipo_usuario=tipo_usuario, 
                           cantidad_carrito=cantidad_carrito) # Pasar el rol a la plantilla
...
def login():
    ...
    if request.method == 'POST':
        ...
        # Lógica para Administrador (usa sesión tradicional)
        ...
        # Lógica para Cliente y Productor (usa Flask-Login)
        usuario = Usuario.query.filter_by(email=email, password=password).first()
        if usuario:
            login_user(usuario)
            session['usuario_id'] = usuario.id  # o nuevo_usuario.id según el caso
            #session['tipo_usuario'] = usuario.tipo_usuario.capitalize()  # o nuevo_usuario.tipo_usuario
...
def panel_cliente():
    ...
    # Ya no se pasan 'tarjetas' a la plantilla.
...
@app.route('/logout')
@login_required # Mantenemos esto si usas Flask-Login
def logout():
    logout_user()  # Cierra la sesión de Flask-Login
    session.clear() # Elimina TODOS los datos de la sesión de Flask
    flash("Has cerrado sesión exitosamente.", "success")

    # Crear una respuesta para redirigir y añadir headers anti-caché
    response = make_response(redirect(url_for('login')))
    response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0, max-age=0'
    response.headers['Pragma'] = 'no-cache' # Para compatibilidad con HTTP/1.0
    response.headers['Expires'] = '-1' # Para proxies
    return response
